...
to_s3 <- function(data, path, id_col = "id") {
  ...
  paste0("https://", Sys.getenv("AWS_S3_ENDPOINT"), "/", path)
}


#' Convenience function to generate mapgl maps
#'
#' @param gdf an `sf` object, URL, or `tbl_duckdb_connection`
#'  to a spatial object
#' @param tooltip column that should be used as the tooltip
#' @param fill_color polygon fill color
#' @param fill_opacity polygon fill opacity
#' @param ... additional parameters to [mapgl::add_fill_layer()]
#' @return a magl map object
#'
